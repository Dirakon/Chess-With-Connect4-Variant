...
public partial class Game : Control
{
    ...
    {
        foreach (var usedHighlighter in UsedHighlighters) usedHighlighter.Modulate = HighlighterDefaultColor;
    ...
    {
        foreach (var hostOnlyView in HostOnlyViews) hostOnlyView.Visible = isHost;
        ResetHighlighters();
        CheckerPositions = new HashSet<Position>();
        InitChessPieces();

        var jsonSettings = JsonConvert.SerializeObject(settings);
        GD.Print($"STARTING GAME WITH [host? {isHost}] [Settings: {jsonSettings}]");
        var isChessPlayer = isHost ? settings.HostPlaysChess : !settings.HostPlaysChess;
    ...
    {
        foreach (var (_, pieceSlot) in _coordsToPieceSlot) pieceSlot.RenderChessPiece(null);
        var chessPieces = new[]
        {
        ...
        _coordsToPiece = new Dictionary<Position, ChessPieceType>();
    ...
    public void HandlePotentialCheckersWin()
    {
        ...
        var verticalLineWin = CheckerPositions
            .Any(checkerPosition =>
                CheckerPositions
                    .Where(position2 =>
        ...
        var horizontalLineWin = CheckerPositions
            .Any(checkerPosition =>
                CheckerPositions
                    .Where(position2 =>
                        position2.Y == checkerPosition.Y
        ...
        var leftDiagonalWin = CheckerPositions
            .Any(checkerPosition =>
                CheckerPositions
                    .Where(position2 =>
                        {
                            var offset = position2 - checkerPosition;
        ...
        var rightDiagonalWin = CheckerPositions
            .Any(checkerPosition =>
                CheckerPositions
                    .Where(position2 =>
                        {
        ...
        if (verticalLineWin || horizontalLineWin || rightDiagonalWin || leftDiagonalWin)
        {
    ...
    public void ChessPlayerMoved(int fromX, int fromY, int toX, int toY)
    {
        switch (_gameState)
        {
            ...
            case ChessPlayerAwaitingOpponentMove:
                GD.PushWarning(
                    $"Somehow initiated a move while awaiting enemy move: from ({fromX}, {fromY}) to ({toX}, {toY})");
                return;
        }
    ...
    public void HandleChessPlayerClick(Position clickedPosition)
    {
        ResetHighlighters();
        {
            ...
            {
                var relevantMove = potentialMoves.FirstOrDefault(move => move.Position == clickedPosition, null);
